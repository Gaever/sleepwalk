...
Проверить выбранные сессии один раз и отправить `продолжай` во все сессии,
которые стоят на лимите.
...
```bash
sleepwalk run-scheduled
```

раз в 30 минут. Если отмеченная сессия остановилась из-за лимита, в неё
отправляется `продолжай`. Указанное агентом время сброса на отправку не влияет.
